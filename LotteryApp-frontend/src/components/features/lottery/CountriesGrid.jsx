const CountriesGrid = ({
  ...
  return (
    <div className="grid grid-cols-4  gap-6 mb-6 justify-items-center min-w-sm">
      {countryConfigs.map((country) => (
        <div key={country.code} className="flex flex-col items-center">
          ...
          <div className="w-10 h-8 mb-2 flex items-center justify-center ">
            <CircleFlag countryCode={country.flag} height={32} />
          </div>
          {/* Country Code */}
          ...
          {/* Number ball*/}
          <button
            className={`w-12 h-12 rounded-full border-2 font-bold text-lg transition-all duration-200 ${countrySelections[country.code]

  ...
  return (
    ...
    >
      ...
      <div className="p-4 bg-white relative" style={{ marginLeft: '32px' }}>
        {/* Top Perforated Edge Effect */}
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-transparent"></div>

        {/* Bottom Perforated Edge Effect */}
        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-transparent"></div>
          ...
          {selections.map((sel, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center p-1">
                {/* Country Flag */}
                <div className="w-6 h-4 mb-1 flex items-center justify-center bg-white rounded shadow-sm border">
                  {country ? (
                    <img
                      src={country.flag}
                      alt={country.code}
